...
export function PaymentProofUpload({ trackingToken }: { trackingToken: string }) {
  ...
  return <main className="min-h-screen bg-surface-warm px-4 py-8"><section className="mx-auto max-w-2xl rounded-3xl border border-line bg-white p-6 shadow-card sm:p-8"><p className="text-xs font-black uppercase tracking-wider text-brand">Manual transfer</p><h1 className="mt-2 text-3xl font-black">Upload payment proof</h1><p className="mt-2 text-sm leading-6 text-ink-muted">Owner verification is required before your order enters the kitchen.</p>{instruction && <div className="mt-6 rounded-2xl bg-dark-muted p-5 text-white"><p className="text-xs font-bold text-white/60">Send exactly</p><p className="mt-1 text-2xl font-black" dir="ltr">{formatEtbMinor(options!.totalMinor)}</p><p className="mt-4 font-black">{instruction.label}</p><p className="mt-1 text-sm">{instruction.accountHolder} · <b dir="ltr">{instruction.accountIdentifier}</b></p>{instruction.instructions && <p className="mt-2 text-sm text-white/70">{instruction.instructions}</p>}</div>}
