...
export function PublicCheckout({ expectedEntry }: { expectedEntry: StoredCart['entry'] }) {
  ...
  return <main className="min-h-screen bg-surface-warm px-4 py-7"><form onSubmit={submit} className="mx-auto grid max-w-5xl gap-5 lg:grid-cols-[1.1fr_.9fr]"><section className="rounded-2xl border border-line bg-white p-5 shadow-card sm:p-7"><p className="text-xs font-black uppercase tracking-wider text-brand">{pickup ? 'Pickup pre-order' : `Table ${cart.context.table?.label ?? ''}`}</p><h1 className="mt-2 text-3xl font-black">Review and pay</h1>{error && <div role="alert" className="mt-5 rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-800">{error}</div>}
