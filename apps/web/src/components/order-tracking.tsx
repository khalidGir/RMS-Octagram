...
interface TrackedOrder { orderNumber: string; status: string; totalMinor: string; currency: string; createdAt: string; lines: Array<{ itemName: string; variantName: string; quantity: number; lineTotalMinor: string }>; statusHistory: Array<{ status: string; createdAt: string }>; }
...
export function OrderTracking({ token }: { token: string }) {
  ...
  return <main className="min-h-screen bg-[#fffaf3] px-4 py-8"><section className="mx-auto max-w-2xl rounded-3xl border border-line bg-white p-6 shadow-card sm:p-8" aria-live="polite"><div className="flex flex-wrap justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-brand">Order #{order.orderNumber}</p><h1 className="mt-2 text-3xl font-black">{awaitingPayment ? 'Waiting for confirmation' : order.status === 'READY' ? 'Your order is ready' : order.status === 'COMPLETED' ? 'Order completed' : 'We have your order'}</h1></div><span className={`h-fit rounded-full px-3 py-2 text-xs font-black ${order.status === 'READY' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'}`}>{order.status.replaceAll('_', ' ')}</span></div>
    {awaitingPayment && <p className="mt-4 rounded-xl bg-amber-50 p-4 text-sm font-semibold text-amber-900">Payment must be confirmed before the order is released to the kitchen.</p>}
    ...
    <div className="mt-7 rounded-2xl bg-muted p-5"><div className="flex justify-between text-lg font-black"><span>Total payable</span><span dir="ltr">{formatEtbMinor(order.totalMinor)}</span></div>{order.lines.map((line) => <div key={`${line.itemName}-${line.variantName}`} className="mt-3 flex justify-between text-sm"><span>{line.quantity} × {line.itemName}</span><span>{formatEtbMinor(line.lineTotalMinor)}</span></div>)}</div>
